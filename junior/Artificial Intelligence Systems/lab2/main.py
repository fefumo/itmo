...
# -------------------- Database (hardcoded) --------------------
GAMES: Set[str] = {
    "disco_elysium","rimworld","witcher3","cyberpunk2077","portal2","dota2","cs2",
    "silksong","hollow_knight","elden_ring","dark_souls3","fortnite","league_of_legends",
    "pubg","minecraft","overwatch","wow","hotline_miami","satisfactory","factorio","hades"
}

GENRES: Set[str] = {
    "rpg","moba","soulslike","shooter","platformer","puzzle","simulation","survival",
    "adventure","roguelike","mmorpg","action"
}

MECHANICS: Set[str] = {
    "crafting","stealth","coop","pvp","loot","building","farming","permadeath"
}

DEVELOPERS: Set[str] = {
    "cdpr","valve","team_cherry","fromsoftware","epic_games","riot_games","pubg_studios",
    "mojang_studios","blizzard_entertainment","dennaton_games","coffee_stain","wube_software",
    "supergiant","zaum","ludeon_studios","wube"
}
...
INDIE_STUDIO: Set[str] = {
    "zaum","ludeon_studios","wube_software","dennaton_games","team_cherry","mojang_studios","wube","coffee_stain"
}
...
def casual_game(game: str) -> bool:
    g = game_genres(game)
    m = game_mechs(game)
    return (("adventure" in g) or ("puzzle" in g) or ("platformer" in g)) and ("pvp" not in m) and ("permadeath" not in m)

# -------------------- User dialog & scoring --------------------
GENRE_ALIASES: Dict[str, str] = {
    "rpg":"rpg","ммо":"mmorpg","mmorpg":"mmorpg","soulslike":"soulslike","соулслайк":"soulslike",
    "action":"action","экшен":"action","moba":"moba","шутер":"shooter","shooter":"shooter",
    "platformer":"platformer","платформер":"platformer","puzzle":"puzzle","головоломка":"puzzle",
    "simulation":"simulation","симулятор":"simulation","survival":"survival","выживание":"survival",
    "adventure":"adventure","приключения":"adventure","roguelike":"roguelike","рогалик":"roguelike"
}
MECH_ALIASES: Dict[str, str] = {
    "crafting":"crafting","крафт":"crafting","stealth":"stealth","стелс":"stealth",
    "coop":"coop","co-op":"coop","кооп":"coop","кооператив":"coop",
    "pvp":"pvp","пвп":"pvp","loot":"loot","лут":"loot","building":"building","строительство":"building",
    "farming":"farming","фермерство":"farming","permadeath":"permadeath","перманентная смерть":"permadeath"
}
...
def dialog() -> Prefs:
    ...
    genres = map_aliases(parse_list(input("Жанры (RPG, shooter, roguelike ...): ")), GENRE_ALIASES)
    mechs  = map_aliases(parse_list(input("Механики (crafting, pvp, coop ...): ")), MECH_ALIASES)
    ...
    return Prefs(age, genres, mechs, want_pvp, want_coop, prefer_indie, dislike_hardcore)
...
def main():
    ...
    for st in sorted(studios):
        ...
        if gspec or mspec:
            show.append((st, ", ".join(sorted(gspec)) or "-", ", ".join(sorted(mspec)) or "-"))
    if show:
        print("\n=== Студии под ваши вкусы ===")
        print(format_table(show, headers=("Студия","Жанры","Механики"), numeric_cols=set()))
