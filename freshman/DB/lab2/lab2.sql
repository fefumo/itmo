...
-- в ответе нет строчек, так как минимальный возраст отрицательный. Это из-за того, что есть люди, которые родились позже существующего года
...
FROM Н_ЛЮДИ
JOIN Н_УЧЕНИКИ ON Н_УЧЕНИКИ.ЧЛВК_ИД = Н_ЛЮДИ.ИД
WHERE Н_УЧЕНИКИ.ГРУППА = '1100' AND Н_УЧЕНИКИ.ДАТА_СМЕРТИ != NULL;
...
FROM(
    ...
    FROM Н_ЛЮДИ
    WHERE Н_УЧЕНИКИ.ДАТА_СМЕРТИ != NULL
) AS SUBQUERY
;
...
SELECT Н_УЧЕНИКИ.ГРУППА, AVG(
    CASE 
        WHEN Н_ЛЮДИ.ДАТА_СМЕРТИ != '9999-09-09' AND Н_ЛЮДИ.ДАТА_СМЕРТИ !=NULL
            THEN EXTRACT (YEAR FROM Н_ЛЮДИ.ДАТА_СМЕРТИ) - EXTRACT (YEAR FROM Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)
        ELSE EXTRACT (YEAR FROM CURRENT_DATE) - EXTRACT (YEAR FROM Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)
    END 
) AS Средний_возраст
