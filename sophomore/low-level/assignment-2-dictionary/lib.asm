...
; if we get here, the character is not whitespace, continue reading the word
.read_loop:
...
; for dict task
    ; rdi has addr of buffer, rsi has buffer's size
read_string:
    
    mov r8, rdi ; addr of buf
    mov r9, rsi ; size (buffer)
    xor r10, r10 ; offest

    .loop:
    call read_char ; rax has the symbol or 0 if eof
    cmp rax, `\n`
    je .terminate
    ...
    mov byte[r8+ r10], al ; put symbol in the buffer

    inc r10

    cmp r10, r9 ; check overflow
    jge .err
    ...
    .terminate:
    mov byte[r8 + r10], 0
    mov rax, r8 ; return addr. can't use rdi directly, because it is overwritten during read_char
    mov rdx, r10 ; return size
    .end:
    ret
