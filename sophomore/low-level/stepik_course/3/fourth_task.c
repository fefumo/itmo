/* вставил прям как и есть в степике. тут всё просто */
...
void interpret_pop   ( struct vm_state* state )  {
    struct maybe_int64 popped = stack_pop(&state->data_stack);
}

void interpret_dup   ( struct vm_state* state )  {
    struct maybe_int64 popped = stack_pop(&state->data_stack);
    stack_push(&state->data_stack, popped.value);
    stack_push(&state->data_stack, popped.value);
}
...
{ 
    struct maybe_int64 val = stack_pop(s);
    int64_t res = f(val.value);
    stack_push(s, res);
}
